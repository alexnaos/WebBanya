...
SettingsGyver sett("БаняМонитор", &db);
...
void build(sets::Builder &b)
{
    ...
    if (b.beginGroup("Group 1"))
    {
        ...
        // b.Input(kk::tmp1, "Temp1");
        // Принудительно превращаем значение из базы в строку
        // b.Label(kk::tmp1, "Темп1", db[kk::tmp1].toString());
        // b.Label(kk::tmp2, "Темп2", db[kk::tmp2].toString());
        b.Label(kk::tmp1, "Темп1");
        b.Label(kk::tmp2, "Темп2");
        b.endGroup();
    }
...
void updateOLED()
{
    // Убираем чтение датчиков отсюда! Используем готовые переменные из loop.
    ...
    // Используем rtc из настроек
    display.print(sett.rtc.timeToString());
    display.print("  ");
...
void loop()
{
    ...
    // --- ТАЙМЕР 1: ЭКРАН (100 мс достаточно) ---
    static uint32_t tmrOLED;
    if (millis() - tmrOLED >= 100)
    {
        tmrOLED = millis();
        updateOLED(); // Рисует значения из db
    }
    ...
    if (millis() - tmrTemp >= 2000)
    {
        ...
        {
            temp1 = t1;
            db[kk::tmp1] = temp1; // ОБЯЗАТЕЛЬНО пишем в базу
        }
        if (t2 != DEVICE_DISCONNECTED_C)
        {
            temp2 = t2;
            db[kk::tmp2] = temp2; // ОБЯЗАТЕЛЬНО пишем в базу
        }

        sensors.requestTemperatures(); // Запрос на следующий цикл
    }
}
